...
def test_question_submission_and_answer_retrieval(client):
    ...
    """
    response = client.post('/api/messages', json={'message': 'What is organic farming?'})
    assert response.status_code == 200
...
def test_question_submission_with_no_document(client):
    ...
    """
    response = client.post('/api/messages', json={'message': 'What is organic farming?'})
    assert response.status_code == 200  # Assuming no document is required for basic questions
    assert 'response' in response.json
